...
/* Word text boxes (e.g. a logo made of 2-3 separately-colored text boxes
   ...
   Note: this only helps when the fragments are real *text*. If the
   header is actually made of floating *images* (as confirmed for this
   project's own invoice template — see handleWordUpload comments below),
   this pass does nothing for it, because there's no text to merge. */
...
export default function DocumentEditorPage() {
  ...
  /* ── Selection persistence ──
     ...
     <Input>, a <Select>, or a toolbar button — the moment focus leaves
     the editor, window.getSelection() collapses/changes and the browser
     "forgets" what was highlighted. We continuously track the last valid
     range inside the editor via the selectionchange event, AND we now
     also snapshot it on every pointerdown/mousedown *inside* the editor
     (capture phase) so a fast click-then-drag-select followed by an
     immediate toolbar click can't race past the selectionchange handler.
     restoreSelection() re-applies that saved range before any formatting
     command runs, so Bold/Italic/color/etc. reliably act on what the
     user actually selected instead of silently landing at the end of
     the document. */
  ...
  /* Toolbar buttons are type="button" by default in a plain <button>,
     but clicking one still moves focus off the editor *before* the
     click handler runs, on some browsers/timings, which is what was
     causing selection to look "unclicked". Using onMouseDown with
     preventDefault (instead of relying only on onClick) stops the
     browser from shifting focus/selection away from the editor at all,
     so the highlighted text stays selected right up until exec() runs. */
  ...
  const handleWordUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      try {
        ...
        // ── Heads-up for logo/letterhead-heavy documents ──
        // If a doc has many images relative to how much real text it
        // has, it's very likely built from floating/absolutely-anchored
        // pictures (a letterhead, a scanned-looking logo block, etc).
        // No HTML converter — mammoth or otherwise — can reliably
        // reproduce Word's exact floating-image canvas positions in a
        // flowing HTML page, because HTML has no equivalent layout
        // model. Import PDF renders actual pixels instead, so it will
        // always match Word exactly for documents like this.
  ...
  /* ── Export ── */
  const exportCSS = `
    body{font-family:Arial;width:210mm;margin:0 auto;padding:25.4mm;line-height:1.5;font-size:14px}
      ...
      <div className="flex h-[calc(100vh-56px)] overflow-hidden bg-background">
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          <div className="flex items-center gap-1 flex-wrap px-2 py-1.5 border-b border-border bg-card select-none">
            ...
            {/* Zoom */}
            <button className={tb(false)} onMouseDown={preserveSelectionOnMouseDown} onClick={() => setPageZoom(z => Math.max(50, z - 10))} title="Zoom out"><ZoomOut className="h-4 w-4"/></button>
            <button onMouseDown={preserveSelectionOnMouseDown} onClick={() => setPageZoom(100)} title="Reset zoom"
              style={{fontSize:12,minWidth:40,textAlign:"center",background:"transparent",border:"none",cursor:"pointer"}}>{pageZoom}%</button>
            <button className={tb(false)} onMouseDown={preserveSelectionOnMouseDown} onClick={() => setPageZoom(z => Math.min(200, z + 10))} title="Zoom in"><ZoomIn className="h-4 w-4"/></button>

            <div className="w-px h-5 bg-border mx-0.5"/>

            {/* Undo / Redo */}
            <button className={tb(false)} onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("undo")} title="Undo"><Undo className="h-4 w-4"/></button>
            <button className={tb(false)} onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("redo")} title="Redo"><Redo className="h-4 w-4"/></button>

            <div className="w-px h-5 bg-border mx-0.5"/>

            {/* Block format */}
            <Select defaultValue="p" onValueChange={applyBlock}>
              <SelectTrigger className="h-7 w-28 text-xs" onMouseDown={preserveSelectionOnMouseDown}><SelectValue placeholder="Format"/></SelectTrigger>
              <SelectContent>
            ...
            <Select value={fontFamily} onValueChange={v => { setFontFamily(v); exec("fontName", v); }}>
              <SelectTrigger className="h-7 w-28 text-xs" onMouseDown={preserveSelectionOnMouseDown}><SelectValue/></SelectTrigger>
              <SelectContent>
            ...
            <Select value={fontSize} onValueChange={v => {
              ...
              <SelectTrigger className="h-7 w-14 text-xs" onMouseDown={preserveSelectionOnMouseDown}><SelectValue/></SelectTrigger>
              <SelectContent>
            ...
            <button className={tb(activeFormats.bold)}        onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("bold")}         title="Bold"><Bold className="h-4 w-4"/></button>
            <button className={tb(activeFormats.italic)}      onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("italic")}       title="Italic"><Italic className="h-4 w-4"/></button>
            <button className={tb(activeFormats.underline)}   onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("underline")}    title="Underline"><Underline className="h-4 w-4"/></button>
            <button className={tb(activeFormats.strikeThrough)} onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("strikeThrough")} title="Strikethrough"><Strikethrough className="h-4 w-4"/></button>

            <div className="w-px h-5 bg-border mx-0.5"/>

            {/* Text color */}
            <div className="relative">
              <button className={tb(false)} title="Text Color" onMouseDown={preserveSelectionOnMouseDown}
              ...
              {showColorPicker && (
                <div className="absolute top-9 left-0 z-50 bg-card border border-border rounded-xl shadow-xl p-2 grid grid-cols-8 gap-1 w-44">
                  {COLORS.map(c => (
                    <button key={c} style={{backgroundColor:c}} onMouseDown={preserveSelectionOnMouseDown}
            ...
            {/* Highlight color */}
            <div className="relative">
              <button className={tb(false)} title="Highlight Color" onMouseDown={preserveSelectionOnMouseDown}
            ...
            {/* Alignment */}
            <button className={tb(activeFormats.justifyLeft)}   onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("justifyLeft")}   title="Align Left"><AlignLeft className="h-4 w-4"/></button>
            <button className={tb(activeFormats.justifyCenter)} onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("justifyCenter")} title="Center"><AlignCenter className="h-4 w-4"/></button>
            <button className={tb(activeFormats.justifyRight)}  onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("justifyRight")}  title="Right"><AlignRight className="h-4 w-4"/></button>
            <button className={tb(activeFormats.justifyFull)}   onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("justifyFull")}   title="Justify"><AlignJustify className="h-4 w-4"/></button>

            <div className="w-px h-5 bg-border mx-0.5"/>

            {/* Lists */}
            <button className={tb(activeFormats.insertUnorderedList)} onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("insertUnorderedList")} title="Bullet List"><List className="h-4 w-4"/></button>
            <button className={tb(activeFormats.insertOrderedList)}   onMouseDown={preserveSelectionOnMouseDown} onClick={() => exec("insertOrderedList")}   title="Numbered List"><ListOrdered className="h-4 w-4"/></button>

            <div className="w-px h-5 bg-border mx-0.5"/>

            {/* Table */}
            <button className={tb(false)} onMouseDown={preserveSelectionOnMouseDown} onClick={() => { restoreSelection(); setShowTableModal(true); }} title="Insert Table"><Table className="h-4 w-4"/></button>

            {/* Image */}
            <button className={tb(false)} onMouseDown={preserveSelectionOnMouseDown} onClick={() => imgRef.current?.click()} title="Insert Image(s)"><ImageIcon className="h-4 w-4"/></button>

            {/* Link */}
            <button className={tb(false)} onMouseDown={preserveSelectionOnMouseDown} onClick={insertLink} title="Insert Link"><Link className="h-4 w-4"/></button>
          </div>
          ...
          {selectedDoc ? (
            <div className="flex-1 overflow-y-auto" style={{background:"#e8eaed"}}>
              <style>{`
                /* Word-style defaults: 1.5 line-height, 8pt paragraph gap,
                   1-inch page margins (applied on the page wrapper below).
                   These match what a fresh Word document uses by default,
                   so new content — and imported content that doesn't
                   specify its own spacing — looks/spaces the same way. */
                #ld-editor{outline:none;min-height:100%;line-height:1.5;}
              ...
              {/* Word-style page: fixed A4/Letter width, centered, 1-inch
                  margins, scales with pageZoom — matches how Word itself
                  lays a page out on screen. */}
              <div style={{ display:"flex", justifyContent:"center", padding:"32px 16px" }}>
                <div style={{
                  ...
                  <div style={{
                    ...
                    padding:"96px",       // 1-inch margins on all sides, Word's default
