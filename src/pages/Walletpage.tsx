...
export default function WalletPage() {
  ...
  const balancePaise = useMemo(() => {
    // only count captured/paid like statuses
    const ok = new Set(["captured", "authorized", "paid", "success"]);
    return txns.reduce((sum, t) => sum + (ok.has((t.status || "").toLowerCase()) ? (t.amount || 0) : 0), 0);
  }, [txns]);
                ...
                <div key={t.id} className="py-3 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-medium truncate">
                      {t.description || "Razorpay Payment"}
                    </p>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <Badge variant={(t.status || "").toLowerCase() === "captured" ? "default" : "secondary"}>
                      {t.status || "unknown"}
                    </Badge>
                  </div>
                </div>
