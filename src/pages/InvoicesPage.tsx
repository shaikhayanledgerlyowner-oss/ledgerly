...
export default function InvoicesPage() {
  ...
  const downloadPDF = async (doc: any) => {
    try {
      ...
      const boxW = 250; const boxX = pageW - 40 - boxW; const boxY = afterTableY; const rowH = 16;
      pdf.setDrawColor(226, 232, 240); pdf.setFillColor(248, 250, 252); pdf.roundedRect(boxX, boxY, boxW, 90, 10, 10, "FD");
      pdf.setTextColor(15, 23, 42); pdf.setFont("helvetica", "normal"); pdf.setFontSize(10);
      pdf.text("Subtotal", boxX + 12, boxY + 24); pdf.text(moneyPDF(Number(totals.subtotal || 0), currency), boxX + boxW - 12, boxY + 24, { align: "right" });
      pdf.text(`Tax (${Number(totals.tax_percent || 0).toFixed(0)}%)`, boxX + 12, boxY + 24 + rowH); pdf.text(moneyPDF(Number(totals.tax_amount || 0), currency), boxX + boxW - 12, boxY + 24 + rowH, { align: "right" });
      pdf.setFont("helvetica", "bold"); pdf.text("Grand Total", boxX + 12, boxY + 24 + rowH * 2 + 8); pdf.text(moneyPDF(Number(totals.total || 0), currency), boxX + boxW - 12, boxY + 24 + rowH * 2 + 8, { align: "right" });
